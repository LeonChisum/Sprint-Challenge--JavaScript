...
const universities = graduates.sort(alphabetically);
...
zooAnimals.forEach((x) => animalNames.push(x.animal_name, x.scientific_name));
...
let lower = (x) => x.animal_name.toLowerCase();
let lowerCase = zooAnimals.map(lower);
...
let count = (x) => x.population < 5;
const lowerPopulation = zooAnimals.filter(count);
...
const populationTotal = zooAnimals.reduce(
  (sum, animals) => (sum += animals.population),0
)
